...
export default function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        const unsubsChukis = onValue(chukipusRef, (snapshot) => {
            ...
            Object.entries(chukipusData).forEach(([chukiId, c]: [string, any]) => {
                if (c.members) {
                    const isTargetMember = Array.isArray(c.members) ? c.members.includes(id) : c.members[id] === true;
                    if (isTargetMember) {
                        chukipuIds.add(chukiId);
                        chukipuMap[chukiId] = c.name;

                        if (user) {
                            const amIMember = Array.isArray(c.members) ? c.members.includes(user.uid) : c.members[user.uid] === true;
            ...
            const unsubsPlans = onValue(plansRef, (plansSnap) => {
                ...
                Object.entries(plansData).forEach(([planId, p]: [string, any]) => {
                    if (chukipuIds.has(p.chukipuId) && p.createdBy === id && p.showInProfile) {
                        ...
                        if (!isProfilePrivate || weFollowThem || imMemberOf.has(p.chukipuId) || isOwnProfile) {
                            allPlans.push({
                                ...p,
                                id: planId,
                                chukipuName: chukipuMap[p.chukipuId] || ''
                            } as ProfilePlan);
                        }
                    }
                });
    ...
    const handleShare = async (plan: ProfilePlan) => {
        const url = `${window.location.origin}/chukipus/${plan.chukipuId}/plans/${plan.id}`;
        if (navigator.share) {
            try {
