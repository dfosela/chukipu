                ...
                Object.values(data).forEach((c: any) => {
                    if (c.members) {
                        if (Array.isArray(c.members)) {
                            if (c.members.includes(user.uid)) count++;
                        } else if (typeof c.members === 'object') {
                            if (c.members[user.uid] === true) count++;
                        }
                    }
                });
    ...
    return (
        <div className={styles.container}>
            ...
            <header className={styles.header}>
                ...
                <button className={styles.headerBtn} onClick={() => router.push('/profile/settings')} aria-label="Configuración">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className={`page hide-scrollbar ${styles.pageContent}`}>
                ...
                <div className={styles.profileHeader}>
                    ...
                    <div className={styles.statsRow}>
                        ...
                        <button className={styles.statItem} onClick={() => router.push('/profile/followers')}>
                            <span className={styles.statValue}>{profile?.followersCount || 0}</span>
                            <span className={styles.statLabel}>Seguidores</span>
                        </button>
                        <div className={styles.divider} />
                        <button className={styles.statItem} onClick={() => router.push('/profile/following')}>
                ...
                <div className={styles.editBtnWrap}>
                    <button className={styles.editProfileBtn} onClick={() => router.push('/profile/edit')}>
                        Editar perfil
                    </button>
                </div>
                    ...
                    {loadingMedia ? (
                        ...
                        groups.map((group) => (
                            <PlanCollage
                                key={group.planId}
                                group={group}
                                onClick={() => router.push(`/chukipus/${group.chukipuId}/plans/${group.planId}`)}
                            />
                        ))
                    )}
                </div>
